 ...
 * more powerful primitives safely. Raw coordinate taps, raw coordinate swipes, and approved shell
 * commands are excluded. App launches are admitted only through ReasoningGroundingPolicy, which
 * requires the exact target to exist in the trusted launchable-app inventory embedded by
 * DeviceController in the inspected observation. Model-supplied package names or labels therefore
 * cannot authorize themselves.
 *
 * Back and Home are capability-admitted here only because ObservedPlanProposal applies the stricter
 * ReasoningIntentBindingPolicy before sealing a proposal. That policy requires explicit navigation
 * language in the user's goal, while ActionVerifier requires a post-navigation UI change. This
 * keeps system navigation unavailable as an incidental recovery maneuver while allowing narrowly
 * requested navigation goals.
 *
...
object ReasoningPlanPolicy {
    ...
    fun evaluate(actions: List<AgentAction>): Decision {
        ...
        return Decision(true, "Reasoning plan stays inside the bounded autonomous vocabulary and one-mutation observation horizon")
    }
