...
object ResumePolicy {
    fun decide(session: PersistedTaskSession?, plan: CommandPlanner.Plan): ResumeDecision {
        ...
        return when (session.status) {
            AgentTaskState.Status.WAITING_CONFIRMATION -> ResumeDecision(true, step, "Resume at confirmation-gated step")
            AgentTaskState.Status.RUNNING,
            AgentTaskState.Status.RECOVERING -> ResumeDecision(true, step, "Resume from first unverified checkpoint")
